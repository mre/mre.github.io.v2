...
First off, let me say that you probably don't want to use this tool as a replacement for `ls` on your local machine (although you could!).
...
-rwxr--r-- 1 mendler  staff  6323935 Mar  8 21:56 Rick Astley - Never Gonna Give You Up.mp3
...
* Next we got the hardlink count of the file, when referring to a file, or the number of contained directory entries, when referring to a directory. ([Reference](https://unix.stackexchange.com/a/43047))
* Owner name
* Group name
...
Let's start with something that prints all files in a directory in Rust:
...
If we run it, we get the expected output:
...
We should stop for a moment here and celebrate our success, knowing that we just wrote our first little Unix utility from scratch.
*Pro Tip*: You can install the binary with `cargo install` and call it like any other binary from there on.
...
Usually, if we type `ls mydir`, we expect to get the file listing of no other directory than `mydir`. We should add the same feature to our clone.
...
```rust
SystemTime { tv_sec: 1520554933, tv_nsec: 610406401 }
```

But the format that we would like to have is something like

```rust
Mar  9 01:24
```

Thankfully, there is a library called `chrono`, which can read this format and [convert it into any human readable format]() we like:
...
This `{:>5}` might jump out. It's a formatting directive provided by [`std::fmt`](https://doc.rust-lang.org/std/fmt/).
...
The file permissions are a bit more tricky.
While the `rwx` file mode is very common in Unix derivatives such as \*BSD or GNU/Linux, many other operating systems ship their own permission management.
...
I just want to demonstrate, that there are a lot of important details to be considered when implementing this in real life.
...
Behind the `r`, the `w` and the `x` are in reality octal numbers. That's easier for computers to work with and many hardcore users prefer the numbers over the symbols.

The ruleset behind those octals is as follows. I took that from the `chmod` manpage, since I couldn't possibly do a better job anyway.
...
So for example, to set the permissions for a file so that the owner can read, write and execute it and nobody else can do anything would be 700 (400 + 200 +100).

Granted, those numbers are the same since the 70s and are not going to change soon, but it's still a bad idea to compare our file permissions directly with the values &mdash; if not for compatibility reasons, then for readability and to avoid magic numbers in our code.

Therefore, we use the `libc` crate, which provides constants for those magic numbers.
As mentioned above, these file permissions are Unix specific, so we need to import a Unix-only library named [`std::os::unix::fs::PermissionsExt;`](https://doc.rust-lang.org/std/os/unix/fs/trait.PermissionsExt.html) as well.
...
`parse_permissions()` is a little helper function defined as this:
...
I'm always surprised about the complexity of even the simplest tools.
